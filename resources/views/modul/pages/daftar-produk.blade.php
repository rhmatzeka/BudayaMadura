        ...
        <a href="{{ route('produk.show', $produk->slug) }}" class="text-decoration-none">
            <div class="product-card">
                @if($produk->gambar->count() > 0)
                    <img src="{{ asset('images/produk/' . $produk->gambar->first()->path_gambar) }}" 
                ...
                @else
                    <div style="height: 250px; background: linear-gradient(135deg, #FFE5B4 0%, #FFD700 100%); display: flex; align-items: center; justify-content: center;">
                        <div class="text-center">
                            ...
                            <p style="color: var(--dark-brown); margin-top: 15px; font-weight: 600; font-size: 1.2rem;">{{ $produk->nama_produk }}</p>
                        </div>
                    </div>
                @endif
                <div class="card-body">
                    <h4 style="color: var(--dark-brown); font-weight: 600; margin-bottom: 15px;">{{ $produk->nama_produk }}</h4>
                    <p style="color: #666; margin-bottom: 15px;">{{ Str::limit($produk->deskripsi, 120) }}</p>
...
<div style="background: linear-gradient(135deg, #E8F4F8 0%, #D4E8F0 100%); padding: 40px; border-radius: 20px; margin-top: 40px;">
    ...
    <div class="row g-3">
        ...
        <div class="col-md-3 col-6">
            <a href="{{ route('produk.show', $produk->slug) }}" class="btn btn-primary-custom w-100" style="padding: 15px;">
                <i class="fas fa-arrow-circle-right me-2"></i>{{ $produk->nama_produk }}
            </a>
        </div>
        @endforeach
    </div>
